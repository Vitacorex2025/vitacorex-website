...
| `corporate-legal-file-control.html` | 792 | button | Estimate cost exposure | `(handler)` | action-script | b2b | no-handler | <button>/[role=button] with no handler wiring |
| `data-handling.html` | 172 | a | Leadership | `/about/leadership.html` | primary-nav | shared | internal-file-missing | about/leadership.html |
| `index.html` | 1041 | button | Close | `(handler)` | action-script | shared | no-handler | <button>/[role=button] with no handler wiring |
| `index.html` | 1530 | button | Generate engagement recommendation | `(handler)` | action-script | shared | no-handler | <button>/[role=button] with no handler wiring |
