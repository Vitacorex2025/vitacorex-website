...
def test_platform_services_catalog_matches_client_facing_domains():
    ...
    assert primary_nav == ["home", "legal", "recovery", "calendar", "document_prep", "my_cases", "downloads", "settings"]
    primary_nav_routes = {item["slug"]: item["route"] for item in payload["primary_navigation"]}
    assert primary_nav_routes["calendar"] == "/app/action-center/"
    assert primary_nav_routes["document_prep"] == "/app/workspace/"
...
def test_platform_dashboard_includes_commercial_snapshot_for_home_and_settings():
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["environment_access"]["default_environment"] == "recovery"
    assert [item["slug"] for item in payload["operating_environments"]] == ["legal", "recovery"]
