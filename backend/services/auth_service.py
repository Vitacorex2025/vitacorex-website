...
LEGAL_TITLE_HINTS = {"attorney", "lawyer", "paralegal", "legal", "litigation", "counsel", "court"}
RECOVERY_TITLE_HINTS = {"collector", "collections", "recovery", "intake", "accounts", "billing", "settlement", "claims"}
MANAGER_TITLE_HINTS = {"manager", "director", "lead", "operations", "ops", "principal", "partner"}
...
def _title_tokens(user: UserRecord) -> set[str]:
    raw = " ".join(
        item
        for item in [getattr(user, "title", ""), getattr(user, "first_name", ""), getattr(user, "last_name", "")]
        if item
    ).strip().lower()
    if not raw:
        return set()
...
def environment_access_payload(user: UserRecord, membership: MembershipRecord) -> dict[str, object]:
    role = str(membership.role or MembershipRole.VIEWER).strip().lower()
    tokens = _title_tokens(user)
    legal_hint = bool(tokens.intersection(LEGAL_TITLE_HINTS))
    recovery_hint = bool(tokens.intersection(RECOVERY_TITLE_HINTS))
    manager_hint = bool(tokens.intersection(MANAGER_TITLE_HINTS))

    available: list[str]
    if _is_admin_role(role) or role == MembershipRole.OWNER or manager_hint:
        available = ["legal", "recovery"]
    elif legal_hint and not recovery_hint:
        available = ["legal"]
    elif recovery_hint and not legal_hint:
        available = ["recovery"]
    else:
        # Generic internal users should be able to switch between both operating
        # environments even when their title does not clearly map to one side.
        available = ["legal", "recovery"]

    if legal_hint and "legal" in available:
        default_environment = "legal"
    elif recovery_hint and "recovery" in available:
        default_environment = "recovery"
    elif "recovery" in available:
        default_environment = "recovery"
    else:
        default_environment = available[0]
    environment_cards = {
        "legal": {
            "slug": "legal",
            "label": "Legal Workflow",
            "description": "Matters, deadlines, hearings, filings, documents, and legal task execution.",
            "route": "/app/legal/",
            "available": "legal" in available,
        },
        "recovery": {
            "slug": "recovery",
            "label": "Recovery Pipeline",
            "description": "Pipeline, contacts, communications, payment follow-up, settlement tracking, and escalation readiness.",
            "route": "/app/recovery/",
            "available": "recovery" in available,
        },
    }
    return {
        "available": available,
        "default_environment": default_environment,
        "switcher_required": len(available) > 1,
        "cards": environment_cards,
    }
...
def session_payload(context: SessionContext) -> dict[str, object]:
    return {
        ...
        "environments": environment_access_payload(context.user, context.membership),
...
def attach_session_cookie(response: Response, session: SessionRecord, request: Request | None = None) -> None:
    kwargs: dict[str, object] = {
        ...
        "expires": utcnow() + timedelta(seconds=SESSION_MAX_AGE_SECONDS),
        "httponly": True,
...
def auth_diagnostics(request: Request | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "resolved_cookie_domain": _cookie_domain_for_request(request),
        "resolved_cookie_secure": _cookie_secure_for_request(request),
        "resolved_cookie_samesite": _cookie_samesite_for_request(request),
    }
