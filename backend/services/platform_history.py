...
def _normalize_environment_tag(value: str | None) -> str:
    normalized = str(value or "").strip().lower()
    if normalized not in {"legal", "recovery"}:
        return "recovery"
    return normalized
...
def _case_summary(record) -> dict[str, Any]:
    ...
    return {
        ...
        "environment_tag": _normalize_environment_tag(getattr(record, "environment_tag", None)),
        "environment_label": "Legal Workflow" if _normalize_environment_tag(getattr(record, "environment_tag", None)) == "legal" else "Recovery Pipeline",
        "record_type": getattr(record, "record_type", None) or ("legal_matter" if _normalize_environment_tag(getattr(record, "environment_tag", None)) == "legal" else "recovery_record"),
        ...
        "linked_records": [item.model_dump(mode="json") for item in (getattr(record, "linked_records", None) or [])],
...
def recent_cases(limit: int = 8, *, environment_tag: str | None = None) -> list[dict[str, Any]]:
    normalized_environment = _normalize_environment_tag(environment_tag) if environment_tag else None
    items: list[dict[str, Any]] = []
    for record in list_cases(limit=max(limit * 4, 12), workspace_id=auth_service.current_workspace_id()):
        if normalized_environment and _normalize_environment_tag(getattr(record, "environment_tag", None)) != normalized_environment:
            continue
        items.append(_case_summary(record))
        if len(items) >= limit:
            break
    return items
...
def _environment_record_count(environment_tag: str) -> int:
    count = 0
    for record in list_cases(limit=500, workspace_id=auth_service.current_workspace_id()):
        if _normalize_environment_tag(getattr(record, "environment_tag", None)) == environment_tag:
            count += 1
    return count


def _environment_urgent_count(items: list[dict[str, Any]], environment_tag: str) -> int:
    if environment_tag == "legal":
        return len([item for item in items if item.get("primary_deadline")])
    return len([item for item in items if item.get("blocker_count") or item.get("warning_count")])


def _operating_environment_payload() -> dict[str, Any]:
    context = auth_service.get_request_context()
    if not context:
        return {"environment_access": {"available": ["recovery"], "default_environment": "recovery", "switcher_required": False, "cards": {}}, "operating_environments": []}

    environment_access = auth_service.environment_access_payload(context.user, context.membership)
    legal_items = recent_cases(limit=4, environment_tag="legal")
    recovery_items = recent_cases(limit=4, environment_tag="recovery")
    environments = []
    for slug, label, description, route, items in (
        ("legal", "Legal Workflow", "Matters, deadlines, hearings, filings, documents, and legal task execution.", "/app/legal/", legal_items),
        ("recovery", "Recovery Pipeline", "Pipeline, contacts, communications, payment follow-up, settlement tracking, and escalation readiness.", "/app/recovery/", recovery_items),
    ):
        environments.append(
            {
                "slug": slug,
                ...
                "route": route,
                "available": slug in (environment_access.get("available") or []),
                "default": slug == environment_access.get("default_environment"),
                "record_count": _environment_record_count(slug),
                "urgent_count": _environment_urgent_count(items, slug),
                "recent_records": items,
            }
        )
    return {"environment_access": environment_access, "operating_environments": environments}


def _quick_actions(environment_access: dict[str, Any]) -> list[dict[str, str]]:
    actions: list[dict[str, str]] = []
    available = list(environment_access.get("available") or [])
    if "legal" in available:
        actions.append(
            {
                "label": "New Legal Matter",
                "description": "Open a legal matter for deadlines, filings, and court-prep workflow.",
                "route": "/app/new-case/?engine=legal",
            }
        )
    if "recovery" in available:
        actions.append(
            {
                "label": "New Recovery Record",
                "description": "Start a recovery record for outreach, payment follow-up, and escalation readiness.",
                "route": "/app/new-case/?engine=recovery&category=dispute",
            }
        )
        actions.append(
            {
                "label": "Start Recovery Intake",
                "description": "Upload documents first, review blockers, then promote into a formal record after review.",
                "route": "/app/case-intake/?workflow=recovery_diagnostic",
            }
        )
    actions.append(
        {
            "label": "Open Matters",
            "description": "Review the shared record list across legal workflow and recovery pipeline.",
            "route": "/app/my-cases/",
        }
    )
    return actions[:4]
...
def home_dashboard_payload() -> dict[str, Any]:
    ...
    operating = _operating_environment_payload()
    return {
        "account": {
            "user_email": context.user.email if context else "",
            "workspace_name": context.workspace.name if context else "",
        },
        "environment_access": operating["environment_access"],
        "operating_environments": operating["operating_environments"],
        "quick_actions": _quick_actions(operating["environment_access"]),
