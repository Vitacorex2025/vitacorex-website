...
def build_platform_navigation() -> list[dict]:
    ...
    if context:
        ...
        available = set(environment_access.get("available") or available)
        default_environment = str(environment_access.get("default_environment") or default_environment)

    navigation: list[dict] = []
    for item in sorted(PRIMARY_NAV_BASE, key=lambda entry: entry["order"]):
        environment = item.get("environment")
        if environment and environment not in available:
            continue
        entry = dict(item)
        if environment:
            entry["is_default_environment"] = environment == default_environment
