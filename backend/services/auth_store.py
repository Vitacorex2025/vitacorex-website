...
from backend.core.config import settings, storage_guard_runtime_error_message
...
# Keep these overridable for tests, but do not initialize the auth directory or
# SQLite file just by importing the module. Render production must be allowed to
# fail its storage guard before any unsafe auth path is touched.
AUTH_ROOT: Path | None = None
AUTH_DB_PATH: Path | None = None


def _auth_root() -> Path:
    return AUTH_ROOT if AUTH_ROOT is not None else settings.storage_path / "auth"


def _auth_db_path() -> Path:
    return AUTH_DB_PATH if AUTH_DB_PATH is not None else _auth_root() / "auth.db"


def _assert_storage_available_for_auth() -> None:
    if not settings.startup_guard_enabled or settings.storage_mount_verified:
        return
    raise RuntimeError(
...
def _legacy_collection_path(name: str) -> Path:
    return _auth_root() / LEGACY_COLLECTIONS[name][1]
...
@contextmanager
def _connect():
    _assert_storage_available_for_auth()
    _auth_root().mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(_auth_db_path(), timeout=30, check_same_thread=False)
