...
class CaseCreateRequest(BaseModel):
    ...
    environment_tag: str | None = Field(default=None, max_length=40)
    record_type: str | None = Field(default=None, max_length=60)
    linked_case_id: str | None = Field(default=None, max_length=60)


class CaseLinkRequest(BaseModel):
    target_case_id: str = Field(min_length=1, max_length=60)
    relationship_type: str = Field(default="linked_operational_record", max_length=60)
...
def _default_record_type(environment_tag: str) -> str:
    return "legal_matter" if environment_tag == "legal" else "recovery_record"


def _normalize_record_type(environment_tag: str, value: str | None) -> str:
    normalized = str(value or "").strip().lower()
    return normalized or _default_record_type(environment_tag)


def _environment_label(environment_tag: str | None) -> str:
    return "Legal Workflow" if _normalize_environment_tag(environment_tag) == "legal" else "Recovery Pipeline"


def _link_summary(record: CaseRecord, relationship_type: str = "linked_operational_record") -> LinkedOperationalRecord:
    environment_tag = _normalize_environment_tag(record.environment_tag)
    return LinkedOperationalRecord(
        case_id=record.id,
        environment_tag=environment_tag,
        record_type=_normalize_record_type(environment_tag, record.record_type),
...
def _sync_linked_records(record: CaseRecord) -> None:
    hydrated: list[LinkedOperationalRecord] = []
    for item in record.linked_records or []:
        try:
            target = load_case(item.case_id)
            hydrated.append(_link_summary(target, relationship_type=item.relationship_type or "linked_operational_record"))
        except FileNotFoundError:
            continue
    record.linked_records = hydrated


def _upsert_link(record: CaseRecord, target: CaseRecord, relationship_type: str = "linked_operational_record") -> None:
    summary = _link_summary(target, relationship_type=relationship_type)
    record.linked_records = [item for item in (record.linked_records or []) if item.case_id != target.id]
    record.linked_records.append(summary)


def _case_summary(record: CaseRecord) -> dict[str, object]:
    _sync_linked_records(record)
    ...
    return {
        ...
        "environment_tag": _normalize_environment_tag(record.environment_tag),
        "environment_label": _environment_label(record.environment_tag),
        "record_type": _normalize_record_type(_normalize_environment_tag(record.environment_tag), record.record_type),
        ...
        "linked_records": [item.model_dump(mode="json") for item in (record.linked_records or [])],
        "linked_record_count": len(record.linked_records or []),
...
@router.get("/cases")
def get_cases(limit: int = 20, include_internal: bool = False, environment_tag: str | None = None) -> list[dict[str, object]]:
    ...
    for record in list_cases(limit=limit, workspace_id=default_workspace_id(), include_internal=include_internal):
        if normalized_environment and _normalize_environment_tag(record.environment_tag) != normalized_environment:
            continue
...
def create_case(payload: CaseCreateRequest, request: Request) -> CaseRecord:
    ...
    environment_tag = _normalize_environment_tag(payload.environment_tag)
    record_type = _normalize_record_type(environment_tag, payload.record_type)
    record = CaseRecord(
        id=case_id,
    ...
    if payload.linked_case_id:
        linked = _load_case_or_404(payload.linked_case_id)
        _upsert_link(record, linked)
        _upsert_link(linked, record)
        save_case(linked)
    save_case(record)
    record_case_event(
        case_id,
        "case.created",
        {
            "reference": record.reference,
            "title": record.title,
            "environment_tag": record.environment_tag,
...
def link_case_records(case_id: str, payload: CaseLinkRequest) -> dict[str, object]:
    source = _load_case_or_404(case_id)
    target = _load_case_or_404(payload.target_case_id)
    _upsert_link(source, target, relationship_type=payload.relationship_type)
    _upsert_link(target, source, relationship_type=payload.relationship_type)
    save_case(source)
    save_case(target)
    record_case_event(
        case_id,
        "case.linked_record.saved",
        {"target_case_id": target.id, "relationship_type": payload.relationship_type},
    )
    record_case_event(
        target.id,
        "case.linked_record.saved",
        {"target_case_id": source.id, "relationship_type": payload.relationship_type},
    )
    return {
        "ok": True,
        "case_id": source.id,
        "target_case_id": target.id,
        "linked_records": [item.model_dump(mode="json") for item in source.linked_records],
    }
