...
from backend.api.routes.private_lookup import router as private_lookup_router
from backend.api.routes.upload import router as upload_router
from backend.core.config import (
    legacy_render_service_block_message,
    settings,
    storage_guard_runtime_error_message,
)
...
HOME_HUB_FILE = APP_ROOT / "home" / "index.html"
HOME_HUB_HTML = HOME_HUB_FILE.read_text(encoding="utf-8") if HOME_HUB_FILE.exists() else ""


def _repo_declares_node_public_web_target() -> bool:
    render_yaml = PROJECT_ROOT / "render.yaml"
    if not render_yaml.exists():
        return False
    text = render_yaml.read_text(encoding="utf-8")
    return (
        "name: docketmint-web" in text
        and "runtime: node" in text
        and "rootDir: tmp_deadline_v10_validation" in text
    )


def _assert_not_stale_public_render_service() -> None:
    if not settings.render_runtime_detected:
        return
    if settings.app_env.lower() != "production":
        return
    if settings.allow_legacy_render_service:
        return
    if not _repo_declares_node_public_web_target():
        return
    raise RuntimeError(
        legacy_render_service_block_message(
            canonical_root_dir="tmp_deadline_v10_validation",
            render_service_name=(os.getenv("RENDER_SERVICE_NAME") or "").strip(),
        )
    )


def _assert_verified_startup_storage() -> None:
    if not settings.startup_guard_enabled:
        return
    if settings.storage_mount_verified:
        return
    raise RuntimeError(
        storage_guard_runtime_error_message(
            resolved_storage_path=settings.storage_path,
            render_disk_mount_path=settings.render_disk_mount_path,
...
async def lifespan(_: FastAPI):
    _assert_not_stale_public_render_service()
    _assert_verified_startup_storage()
...
app.include_router(private_lookup_router, prefix=settings.api_v1_prefix)
...
# Prefixes: any path *starting with* these is treated as public (no DocketMint auth required).
# Private-lookup has its own payment-gate auth; it must be reachable without a session cookie.
PUBLIC_API_PREFIXES = (
    f"{settings.api_v1_prefix}/private-lookup",
    f"{settings.api_v1_prefix}/vitacorex/intake",
)
...
PUBLIC_APP_PREFIXES = {
    ...
    "/app/sign-up",
    "/app.html",
    "/app/private-lookup",  # has its own payment gate — no DocketMint login required
}


def _is_public_api_path(path: str) -> bool:
    normalized = path.rstrip("/") or path
    if normalized in {item.rstrip("/") for item in PUBLIC_API_PATHS}:
        return True
...
def _is_public_app_path(path: str) -> bool:
    normalized = path.rstrip("/") or path
    return any(normalized == p.rstrip("/") or normalized.startswith(p.rstrip("/") + "/")
               for p in PUBLIC_APP_PREFIXES)
...
def _serve_home_hub_html():
    return HTMLResponse(
        HOME_HUB_HTML or _app_file("home", "index.html").read_text(encoding="utf-8"),
...
def _post_login_app_target(request: Request | None = None) -> str:
    context = auth_service.get_request_context()
    if not context and request is not None:
        context = auth_service.resolve_request_context(request)
    if not context:
        return "/app/home/"
    environments = auth_service.environment_access_payload(context.user, context.membership)
    if environments.get("switcher_required"):
        return "/app/home/"
    default_environment = str(environments.get("default_environment") or "recovery").strip().lower()
    if default_environment == "legal":
        return "/app/legal/"
    return "/app/recovery/"
...
def api_system_status() -> dict[str, object]:
    ...
    auth_db_path = auth_store.database_path()
    return {
        "status": "ok",
        ...
        "render_disk_mount_path": str(settings.render_disk_mount_path) if settings.render_disk_mount_path else None,
        "storage_path_within_render_disk_mount": settings.storage_path_within_render_disk_mount,
        "storage_mount_verified": settings.storage_mount_verified,
        "startup_guard_enabled": settings.startup_guard_enabled,
        "storage_exists": storage_path.exists(),
        "auth_db_path": str(auth_db_path),
        "auth_db_exists": auth_store.database_exists(),
        "files_base": "/files",
...
@app.get("/app/", include_in_schema=False)
def serve_app_root(request: Request):
    target = _post_login_app_target(request)
    if target == "/app/home/":
        return _serve_home_hub_html()
...
def serve_sign_in(request: Request):
    if auth_service.get_request_context() or auth_service.resolve_request_context(request):
        return RedirectResponse(url=_post_login_app_target(request), status_code=307)
...
@app.get("/app/legal", include_in_schema=False)
@app.get("/app/legal/", include_in_schema=False)
def serve_legal():
    return _serve_app_html("legal", "index.html")


@app.get("/app/recovery", include_in_schema=False)
@app.get("/app/recovery/", include_in_schema=False)
def serve_recovery():
    return _serve_app_html("recovery", "index.html")
