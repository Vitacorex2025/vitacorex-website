...
def _signup_payload(email: str, language: str = "en") -> dict[str, object]:
    return {
        "first_name": "Test",
        "last_name": "User",
        "email": email,
...
def authenticated_client(app, *, email: str | None = None, language: str = "en") -> TestClient:
    client = TestClient(app)
    payload = _signup_payload(email or f"test-{uuid4().hex[:12]}@example.com", language=language)
    response = client.post("/api/auth/signup", json=payload)
    assert response.status_code == 200, response.text
    if _use_bootstrap_admin():
        _elevate_session_membership_to_admin(client)
    return client


def signup_httpx_client(base_url: str, *, email: str | None = None, language: str = "en") -> httpx.Client:
    client = httpx.Client(base_url=base_url, follow_redirects=True, timeout=60.0)
    payload = _signup_payload(email or f"live-{uuid4().hex[:12]}@example.com", language=language)
    response = client.post("/api/auth/signup", json=payload)
    assert response.status_code == 200, response.text
    if _use_bootstrap_admin():
        _elevate_session_membership_to_admin(client)
