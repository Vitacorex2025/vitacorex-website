...
from backend.core.errors import validation_exception_handler
...
def test_recovery_intake_upload_validation_returns_json_422_without_server_error(admin_rbac_env):
    with TestClient(app) as client:
        _signup(client, f"validation-{uuid4().hex[:10]}@example.com")
        created = client.post(
            "/api/recovery-intake/sessions",
            data={"entry_workflow": "recovery_diagnostic"},
            files=[_text_upload("validation-source.txt")],
        )
        assert created.status_code == 200, created.text
        session_id = created.json()["session_id"]

        response = client.post(f"/api/recovery-intake/sessions/{session_id}/source-files")
        ...
        assert payload["details"][0]["loc"][-1] == "files"


def test_validation_error_handler_sanitizes_uploadfile_objects_for_json_response():
    request = Request(
        {
            "type": "http",
            "method": "POST",
            "scheme": "http",
            "path": "/api/recovery-intake/sessions",
            "query_string": b"",
            "headers": [],
    ...
    request.state.request_id = "req_upload_validation"
    upload = UploadFile(
        file=BytesIO(b"invoice body"),
    ...
    assert payload["error"]["type"] == "validation_error"
    assert payload["details"][0]["input"]["filename"] == "invoice.txt"
    assert payload["details"][0]["input"]["content_type"] == "text/plain"
