(function(){
  ...
  function renderMatterSummary(record){
    ...
    summary.innerHTML =
      '<div class="summary-grid">' +
        ...
        infoCard(t('module.summary.amount', 'Approved amount'), money(snapshot.display_amount || 0)) +
  ...
  function renderDamages(record){
    ...
    detail.innerHTML =
      '<div class="summary-grid">' +
        infoCard(t('module.damages.approved', 'Approved total'), money(reconciliation.approved_reconciled_total || reconciliation.approved_total || 0)) +
        infoCard(t('module.damages.case_total', 'Case summary total'), money(reconciliation.case_summary_total || 0)) +
      ...
      '<div class="module-list-block"><h3>' + escapeHtml(t('module.damages.ledger_preview', 'Ledger preview')) + '</h3>' +
        (lineItems.length ? '<div class="stack-list">' + lineItems.slice(0, 8).map(function(item){
          return '<div><strong>' + escapeHtml(item.invoice_number || item.source_filename || '-') + '</strong><p>' + escapeHtml(money(item.balance_due || 0)) + '</p></div>';
  ...
  function renderPreSuit(record, registry, packet){
    ...
    detail.innerHTML =
      '<div class="summary-grid">' +
        infoCard(t('module.pre_suit.amount', 'Supported balance'), summary.supported_balance ? money(summary.supported_balance) : money(record && record.client_snapshot ? record.client_snapshot.display_amount || 0 : 0)) +
