  ...
  function stageMeta(stage){
    ...
    if(normalized === 'upload'){
      return {
        ...
        summary: 'Upload files and open the recovery matter.',
        order: 1
      };
    }
    ...
    root.innerHTML = groups.map(function(group){
      return '<section class="service-stage-block">' +
        '<div class="subsection-header subsection-header--tight">' +
          '<div><div class="eyebrow">' + escapeHtml('Recovery funnel') + '</div><h2>' + escapeHtml(group.stage.label) + '</h2><p class="service-stage-summary">' + escapeHtml(group.stage.summary) + '</p></div>' +
        '</div>' +
        '<div class="service-catalog-grid">' +
          group.items.map(function(item){
            ...
            features.push(item.entry_mode === 'standalone_session' ? 'Internal module' : (item.entry_mode === 'global_utility' ? 'Utility' : 'Recovery matter'));
            ...
            return '<article class="service-card-compact is-tappable" data-service-route="' + escapeHtml(href) + '" tabindex="0" role="link">' +
              ...
              '<div class="inline-actions"><a class="btn btn-secondary btn-sm" href="' + escapeHtml(href) + '">' + escapeHtml('Open') + '</a></div>' +
            '</article>';
          }).join('') +
        '</div>' +
      '</section>';
