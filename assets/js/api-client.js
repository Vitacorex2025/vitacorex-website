(function(){
  ...
  window.DocketMintApi = {
    ...
    listCases: function(limitOrOptions, environmentTag){
      var limit = 20;
      var env = environmentTag || '';
      if(typeof limitOrOptions === 'object' && limitOrOptions){
        limit = Number(limitOrOptions.limit || 20);
        env = String(limitOrOptions.environmentTag || limitOrOptions.environment_tag || env || '');
      } else {
        limit = Number(limitOrOptions || 20);
      }
      var query = ['limit=' + encodeURIComponent(limit || 20)];
      if(env){
        query.push('environment_tag=' + encodeURIComponent(env));
      }
      return request('/cases?' + query.join('&'));
    },
    createCase: function(payload){ return request('/cases', { method: 'POST', body: payload }); },
    linkCases: function(caseId, payload){ return request('/cases/' + encodeURIComponent(caseId) + '/links', { method: 'POST', body: payload || {} }); },
