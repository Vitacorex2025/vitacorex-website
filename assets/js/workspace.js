(function(){
  ...
  function updateCreateModeUI(){
    ...
    var categoryCopy = {
      ...
      dispute: 'Create a clean recovery matter, then continue in Action Center to confirm the evidence picture, next step, and demand readiness before opening advanced workspace tools.',
    ...
    if(els.intakeCategory){
      els.intakeCategory.textContent = categoryTitle[category] || 'General matter';
    }
    if(!label && !subcopy && !legacyBanner){ return; }
    if(isCreateMode()){
      if(label){ label.textContent = category ? 'Create New Matter' : 'Create New Case'; }
      if(subcopy){ subcopy.textContent = categoryCopy[category] || 'Create mode starts clean. This screen ignores any stored active case until a new matter is successfully created.'; }
      if(legacyBanner){ legacyBanner.textContent = category ? (categoryTitle[category] || 'Matter intake') : 'Create mode starts clean.'; }
      return;
    }
    if(isEditMode()){
    ...
    if(els.createForm){
      els.createForm.addEventListener('submit', async function(event){
        ...
        try {
          ...
          state.currentCaseId = created.id;
          setStoredActiveCaseId(created.id);
          showToast('Matter created. Opening Action Center...', 'success');
          window.location.href = withCase('../action-center/', created.id);
        } catch (error){
          showToast(error.message || 'Could not create matter.', 'warning');
        }
      });
    }
    safeArray(els.reviewForms).forEach(function(form){
