(function(){
  ...
  function stageMeta(stage){
    ...
    if(normalized === 'upload'){
      return {
        ...
        summary: 'Upload files and open the operator matter.',
        order: 1
      };
    }
    if(normalized === 'diagnose'){
      return {
        ...
        summary: 'See blockers, evidence gaps, and readiness before generation.',
        order: 2
      };
    }
    return {
      ...
      summary: 'Build the next practical step once the matter is ready.',
      order: 3
    };
  }
  ...
  function render(){
    ...
    if(!groups.length){
      root.innerHTML = '<div class="empty-copy">' + escapeHtml(t('services.empty', 'No operator workflows are available.')) + '</div>';
      return;
    }
    root.innerHTML = groups.map(function(group){
      return '<section class="service-stage-block">' +
        '<div class="subsection-header subsection-header--tight">' +
          '<div><div class="eyebrow">' + escapeHtml('Operator funnel') + '</div><h2>' + escapeHtml(group.stage.label) + '</h2><p class="service-stage-summary">' + escapeHtml(group.stage.summary) + '</p></div>' +
        '</div>' +
        '<div class="service-catalog-grid">' +
          group.items.map(function(item){
            ...
            features.push(item.entry_mode === 'standalone_session' ? 'Internal module' : (item.entry_mode === 'global_utility' ? 'Utility' : 'Operator matter'));
            if(item.upload_supported){ features.push('Upload supported'); }
            if(item.output_supported){ features.push('Output supported'); }
  ...
  async function boot(){
    try {
      ...
      if(banner){
        banner.hidden = false;
        banner.textContent = error.message || t('services.error.load', 'Could not load the service catalog.');
      }
    }
  }
