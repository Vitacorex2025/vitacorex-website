(function(){
  ...
  function renderFilters(){
    ...
    if(serviceSelect){
      ...
      serviceSelect.innerHTML = '<option value="">' + escapeHtml(t('history.filter_all_services', 'All services')) + '</option>' + services.map(function(item){
    ...
    if(ownerSelect){
      ...
      ownerSelect.innerHTML = '<option value="">' + escapeHtml(t('history.filter_all_owners', 'All matters and sessions')) + '</option>' + owners.map(function(item){
  ...
  function renderCaseList(){
    ...
    if(summary){
      summary.textContent = caseIdsSelected().length + ' ' + t('history.selected_suffix', 'selected');
    }
    if(!items.length){
      root.innerHTML = '<div class="empty-copy">' + escapeHtml(t('history.empty_cases', 'No cases yet.')) + '</div>';
      return;
    }
    root.innerHTML = items.map(function(item){
      ...
      return '<article class="history-row' + (item.id === state.selectedCaseId ? ' is-active' : '') + '">' +
        '<div class="inline-actions">' +
          '<label><input type="checkbox" data-case-select="' + escapeHtml(item.id) + '"' + checked + '> ' + escapeHtml(t('history.select_case', 'Select')) + '</label>' +
        '</div>' +
        '<button class="btn btn-secondary btn-sm" type="button" data-case-row="' + escapeHtml(item.id) + '">' + escapeHtml(t('common.open', 'Open')) + '</button>' +
  ...
  function renderStandaloneSessions(){
    ...
    if(!items.length){
      root.innerHTML = '<div class="empty-copy">' + escapeHtml(t('history.empty_sessions', 'No standalone sessions yet.')) + '</div>';
      return;
    }
    root.innerHTML = items.map(function(item){
      return '<article class="dashboard-list-item">' +
        ...
        '<div class="inline-actions">' +
          '<a class="btn btn-secondary btn-sm" href="' + escapeHtml(item.route) + '">' + escapeHtml(t('common.open', 'Open')) + '</a>' +
          '<button class="btn btn-secondary btn-sm" type="button" data-session-delete="' + escapeHtml(item.service_slug + ':' + item.id) + '">' + escapeHtml(t('common.delete', 'Delete')) + '</button>' +
        '</div>' +
      '</article>';
    }).join('');
    Array.prototype.slice.call(root.querySelectorAll('[data-session-delete]')).forEach(function(button){
      button.addEventListener('click', async function(){
        ...
        if(!window.confirm(t('history.delete_session_confirm', 'Delete this standalone session and its generated files?'))){ return; }
        try {
          ...
          window.alert(error.message || t('history.delete_session_failed', 'Could not delete the standalone session.'));
        }
      });
    });
  }
  ...
  function renderOutputsHistory(){
    ...
    if(summary){
      summary.textContent = historySelections().length + ' ' + t('history.history_selected_suffix', 'history items selected');
    }
    if(!items.length){
      root.innerHTML = '<div class="empty-copy">' + escapeHtml(t('history.empty_filtered_outputs', 'No outputs match the current filters.')) + '</div>';
      return;
    }
    root.innerHTML = items.map(function(item){
      ...
      return '<article class="dashboard-list-item">' +
        '<div class="inline-actions"><label><input type="checkbox" data-history-select="' + escapeHtml(item.id) + '"' + checked + '> ' + escapeHtml(t('common.select', 'Select')) + '</label></div>' +
        ...
        '<div class="inline-actions"><a class="btn btn-secondary btn-sm" href="' + escapeHtml(href) + '">' + escapeHtml(t('common.open', 'Open')) + '</a></div>' +
      '</article>';
    }).join('');
  ...
  function renderCaseDetail(){
    ...
    if(!record){
      if(title){ title.textContent = t('history.detail_title', 'Select a case'); }
      if(status){ status.textContent = t('history.detail_status', 'Choose a matter to manage its source files and outputs.'); }
      if(summary){ summary.innerHTML = '<div class="empty-copy">' + escapeHtml(t('history.empty_case_summary', 'Case detail appears here.')) + '</div>'; }
      if(sources){ sources.innerHTML = '<div class="empty-copy">' + escapeHtml(t('history.no_case_selected', 'No case selected.')) + '</div>'; }
      if(outputs){ outputs.innerHTML = '<div class="empty-copy">' + escapeHtml(t('history.no_case_selected', 'No case selected.')) + '</div>'; }
    ...
    if(summary){
      ...
      summary.innerHTML =
        '<div class="summary-grid">' +
          '<div class="summary-item"><span>' + escapeHtml(t('history.summary_source_files', 'Source files')) + '</span><strong>' + escapeHtml(snapshot.source_file_count || 0) + '</strong></div>' +
          '<div class="summary-item"><span>' + escapeHtml(t('history.summary_outputs', 'Outputs')) + '</span><strong>' + escapeHtml(snapshot.generated_document_count || 0) + '</strong></div>' +
          '<div class="summary-item"><span>' + escapeHtml(t('history.summary_blockers', 'Blockers')) + '</span><strong>' + escapeHtml(snapshot.blocker_count || 0) + '</strong></div>' +
          '<div class="summary-item"><span>' + escapeHtml(t('history.summary_warnings', 'Warnings')) + '</span><strong>' + escapeHtml(snapshot.warning_count || 0) + '</strong></div>' +
        '</div>';
    }
    if(actions){
      actions.innerHTML =
        '<a class="btn btn-secondary btn-sm" href="/app/action-center/?case=' + escapeHtml(record.id) + '">' + escapeHtml(t('history.open_action_center', 'Open Action Center')) + '</a>' +
        '<a class="btn btn-secondary btn-sm" href="/app/workspace/?case=' + escapeHtml(record.id) + '">' + escapeHtml(t('common.open_workspace', 'Open workspace')) + '</a>' +
        '<button class="btn btn-secondary btn-sm" type="button" data-case-delete>' + escapeHtml(t('common.delete', 'Delete')) + '</button>';
      var deleteButton = actions.querySelector('[data-case-delete]');
      if(deleteButton){
        deleteButton.addEventListener('click', async function(){
          if(!window.confirm(t('history.delete_selected_confirm', 'Delete the selected cases and all files inside them?'))){ return; }
          try {
            await api.deleteCase(record.id, true);
            setSelectedCase('');
            await boot();
          } catch (error){
            window.alert(error.message || t('history.load_failed', 'Could not load case history.'));
          }
        });
      }
    }
    if(sources){
      ...
      sources.innerHTML = files.length ? files.map(function(item){
        return '<article class="dashboard-list-item">' +
          ...
          '<div class="inline-actions"><button class="btn btn-secondary btn-sm" type="button" data-source-delete="' + escapeHtml(item.id) + '">' + escapeHtml(t('history.remove_file', 'Remove file')) + '</button></div>' +
        '</article>';
      }).join('') : '<div class="empty-copy">' + escapeHtml(t('history.empty_source_files', 'No source files in this matter.')) + '</div>';
      Array.prototype.slice.call(sources.querySelectorAll('[data-source-delete]')).forEach(function(button){
        button.addEventListener('click', async function(){
          if(!window.confirm(t('history.remove_file_confirm', 'Remove this uploaded file from the matter?'))){ return; }
          try {
            await api.removeSourceFile(record.id, button.getAttribute('data-source-delete'));
            await loadCaseDetail();
            await refreshHistory();
          } catch (error){
            window.alert(error.message || t('history.remove_file_failed', 'Could not remove the source file.'));
          }
        });
      });
    }
    if(outputs){
      ...
      outputs.innerHTML = entries.length ? entries.map(function(item){
        return '<article class="dashboard-list-item">' +
          ...
          '<div class="inline-actions">' +
            (item.download_url ? '<a class="btn btn-secondary btn-sm" href="' + escapeHtml(api.toDownloadUrl(item.download_url)) + '">' + escapeHtml(t('common.download', 'Download')) + '</a>' : '') +
            '<button class="btn btn-secondary btn-sm" type="button" data-output-delete="' + escapeHtml(item.id) + '">' + escapeHtml(t('history.remove_output', 'Remove output')) + '</button>' +
          '</div>' +
        '</article>';
      }).join('') : '<div class="empty-copy">' + escapeHtml(t('history.empty_outputs', 'No generated outputs in this matter.')) + '</div>';
      Array.prototype.slice.call(outputs.querySelectorAll('[data-output-delete]')).forEach(function(button){
        button.addEventListener('click', async function(){
          if(!window.confirm(t('history.remove_output_confirm', 'Remove this generated output from the matter?'))){ return; }
          try {
            await api.removeCaseOutput(record.id, button.getAttribute('data-output-delete'));
            await loadCaseDetail();
            await refreshHistory();
          } catch (error){
            window.alert(error.message || t('history.remove_output_failed', 'Could not remove the generated output.'));
          }
        });
      });
    }
  ...
  function bindActions(){
    ...
    if(deleteSelected){
      deleteSelected.addEventListener('click', async function(){
        ...
        if(!ids.length){
          window.alert(t('history.select_case_first', 'Select at least one case first.'));
          return;
        }
        if(!window.confirm(t('history.delete_selected_confirm', 'Delete the selected cases and all files inside them?'))){ return; }
    ...
    if(deleteAll){
      deleteAll.addEventListener('click', async function(){
        if(!window.confirm(t('history.delete_all_confirm', 'Delete all visible cases in this account?'))){ return; }
    ...
    if(clearSelectedHistory){
      clearSelectedHistory.addEventListener('click', async function(){
        ...
        if(!items.length){
          window.alert(t('history.select_history_first', 'Select at least one history item first.'));
          return;
        }
        if(!window.confirm(t('history.clear_selected_confirm', 'Clear the selected output history items?'))){ return; }
    ...
    if(clearHistory){
      clearHistory.addEventListener('click', async function(){
        if(!window.confirm(t('history.clear_history_confirm', 'Clear the current output history view?'))){ return; }
  ...
  async function boot(){
    try {
      ...
      if(banner){
        banner.hidden = false;
        banner.textContent = error.message || t('history.load_failed', 'Could not load case history.');
      }
    }
  }
