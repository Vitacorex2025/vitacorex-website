(function(){
  ...
  function statusLabel(value){
    var map = {
      extracted_successfully: t('intake.status.extracted_successfully', 'Extracted successfully'),
      partially_extracted: t('intake.status.partially_extracted', 'Partially extracted'),
      failed: t('intake.status.failed', 'Failed'),
      pending: t('intake.status.pending', 'Pending')
    };
  ...
  function humanType(value){
    var map = {
      contract_pdf: t('intake.file_type.contract_pdf', 'Contract PDF'),
      guaranty_pdf: t('intake.file_type.guaranty_pdf', 'Guaranty PDF'),
      invoice_excel: t('intake.file_type.invoice_excel', 'Invoice Excel'),
      invoice_csv: t('intake.file_type.invoice_csv', 'Invoice CSV'),
      invoice_pdf: t('intake.file_type.invoice_pdf', 'Invoice PDF'),
      account_statement_pdf: t('intake.file_type.account_statement_pdf', 'Account Statement PDF'),
      supporting_exhibit: t('intake.file_type.supporting_exhibit', 'Supporting Exhibit')
    };
  ...
  function renderSummary(){
    ...
    if(statusNode){ statusNode.textContent = payload.status || t('intake.status_draft', 'Draft'); }
    if(readinessNode){ readinessNode.textContent = payload.readiness || t('intake.readiness_waiting', 'Waiting on files'); }
    if(fileCountNode){ fileCountNode.textContent = String(payload.source_file_count || 0); }
    if(nextStepNode){ nextStepNode.textContent = snapshot.next_action || diagnosis.next_action || t('intake.next_step_upload', 'Upload the source set'); }

    var summaryNode = q('[data-intake-diagnosis-summary]');
    if(summaryNode){
      summaryNode.innerHTML =
        '<strong>' + escapeHtml(snapshot.next_action || diagnosis.next_action || t('intake.diagnosis_default_title', 'Upload files to start diagnosis.')) + '</strong>' +
        '<span>' + escapeHtml((diagnosis.blocker_count ? t('intake.diagnosis_blockers_copy', 'Blockers are listed below before any formal matter can be promoted.') : t('intake.diagnosis_no_blockers_copy', 'No blocker text is available yet. Upload the source set to see readiness and evidence gaps.'))) + '</span>';
    }
  ...
  function renderFiles(){
    ...
    if(!files.length){
      root.innerHTML = '<p class="empty-copy">' + escapeHtml(t('intake.files_empty', 'No source files uploaded yet.')) + '</p>';
      return;
    }
    root.innerHTML = files.map(function(file){
      ...
      return '<article class="stored-item">' +
        '<div class="upload-progress-head"><strong>' + escapeHtml(file.original_filename || file.filename || t('intake.file_source_default', 'Source file')) + '</strong><span>' + escapeHtml([humanType(file.file_type), formatBytes(file.size_bytes || 0)].join(' | ')) + '</span></div>' +
        '<div class="stored-item-qa">' +
          ...
          '<span class="micro-note">' + escapeHtml(t('common.quality', 'Quality')) + ' ' + escapeHtml(String(score)) + '%</span>' +
          '<span class="micro-note">' + escapeHtml(t('common.method', 'Method')) + ' ' + escapeHtml(String(file.extraction_method || 'direct_text').replace(/_/g, ' ')) + '</span>' +
        '</div>' +
        (sheets.length ? '<div class="micro-note"><strong>' + escapeHtml(t('common.worksheet_selector', 'Worksheet selector')) + ':</strong> ' + escapeHtml(sheets.join(' | ')) + '</div>' : '') +
  ...
  function renderMessages(){
    ...
    if(blockers.length){
      ...
      emptyList(blockersNode, t('intake.blockers_empty', 'No blockers detected yet.'));
    }
    if(warnings.length){
      warningsNode.innerHTML = warnings.map(function(item){ return '<li>' + escapeHtml(item) + '</li>'; }).join('');
    } else {
      emptyList(warningsNode, t('intake.warnings_empty', 'No warnings detected yet.'));
    }
  }
  ...
  function renderDeadlines(){
    ...
    deadlines.forEach(function(item){
      rows.push(
        '<article class="dashboard-list-item">' +
          '<div><strong>' + escapeHtml(item.label || t('common.deadline', 'Deadline')) + '</strong><p>' + escapeHtml([item.date || t('common.review_now', 'Review now'), item.urgency || t('intake.deadline_medium', 'medium')].join(' | ')) + '</p></div>' +
        '</article>'
      );
    });
    contacts.slice(0, 3).forEach(function(item){
      rows.push(
        '<article class="dashboard-list-item">' +
          '<div><strong>' + escapeHtml(item.authority_name || t('intake.official_contact', 'Official contact')) + '</strong><p>' + escapeHtml([item.department_name || '', item.phone || '', item.webform || ''].filter(Boolean).join(' | ')) + '</p></div>' +
        '</article>'
      );
    });
    root.innerHTML = rows.length ? rows.join('') : '<p class="empty-copy">' + escapeHtml(t('intake.deadlines_empty', 'No deadlines or official contact paths detected yet.')) + '</p>';
  }
  ...
  async function uploadFiles(files){
    ...
    try {
      ...
      setBanner(error.message || t('intake.upload_failed', 'Could not upload the intake source set.'));
    } finally {
      state.loading = false;
      renderSummary();
    }
  ...
  async function promoteSession(){
    ...
    try {
      ...
      setBanner(error.message || t('intake.promote_failed', 'Could not promote the intake into a formal matter.'));
    } finally {
      state.loading = false;
      renderSummary();
    }
  ...
  async function boot(){
    ...
    if(sessionId){
      try {
        ...
        state.payload = null;
        replaceSessionInUrl('');
        setBanner(t('intake.previous_unavailable', 'The previous intake session is no longer available. Start a new upload-first intake.'));
      }
    }
    render();
    if(params().get('capture') === '1'){
