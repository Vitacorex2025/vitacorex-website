(function(){
  ...
  function buildEnvironmentSwitcher(session){
    var environments = (session && session.environments) || {};
    var available = environments.available || [];
    if(available.length < 2){ return ''; }
    var cards = environments.cards || {};
    var items = [
      { slug: 'home', label: 'Hub', route: '/app/home/', description: 'Unified home' }
    ];
    available.forEach(function(slug){
      var card = cards[slug] || {};
      items.push({
        slug: slug,
        label: card.label || (slug === 'legal' ? 'Legal' : 'Recovery'),
        route: card.route || (slug === 'legal' ? '/app/legal/' : '/app/recovery/'),
        description: card.description || ''
      });
    });
    return '<nav class="environment-switcher" aria-label="Operating environment switcher">' +
      items.map(function(item){
        var itemPath = normalizePath(item.route);
        var active = currentPath() === itemPath;
        return '<a class="environment-switcher-link' + (active ? ' active' : '') + '" href="' + escapeHtml(item.route) + '" title="' + escapeHtml(item.description || item.label) + '">' +
  ...
  function buildAccountShell(session, locale){
    ...
    return '' +
      '<div class="topbar-account-shell" data-auth-shell>' +
        buildEnvironmentSwitcher(session) +
  ...
  function applySession(session){
    ...
    document.body.setAttribute('data-default-environment', (authorization.environments && authorization.environments.default_environment) || 'recovery');
    document.body.setAttribute('data-environment-access', ((authorization.environments && authorization.environments.available) || ['recovery']).join(','));
