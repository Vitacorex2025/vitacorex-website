...
(function(){
  ...
  function initMobileMenu(){
    ...
    function setOpen(open){
      ...
      // Scroll-lock: overflow hidden + touch prevention (no position changes = no jump)
      if(isOpen){
        body.style.overflow = 'hidden';
        root.style.overflow = 'hidden';
        // iOS: prevent touchmove from scrolling body behind the glass overlay
        if(!header._vcxTouchLock){
          header._vcxTouchLock = function(e){
            // Allow scroll inside nav if it overflows
            if(e.target.closest && e.target.closest('.vcx-mobile-nav')) return;
            e.preventDefault();
          };
        }
        doc.addEventListener('touchmove', header._vcxTouchLock, {passive:false});
      } else {
        body.style.overflow = '';
        root.style.overflow = '';
        if(header._vcxTouchLock){
          doc.removeEventListener('touchmove', header._vcxTouchLock);
        }
      }
    }
